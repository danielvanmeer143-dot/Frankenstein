...
function documentLoader() {
  ...
  var PercyArray = Array.from(visible_percy);
    ...
    //write an forEach() method that shows all the text written and modified by both hand (in black?). The forEach() method of Array instances executes a provided function once for each array element.
      MaryArray.forEach(function (span) {
        span.style.color = 'black';
      } )
      PercyArray.forEach(function (span) {
        span.style.color = 'black';        
      } ) 
  ...
  function toggleDeletions() {
    document.querySelectorAll("del").forEach(del => {
      del.classList.toggle("hidden-deletion");
    });
  }
  document
  .getElementById('toggle-deletions')
  .addEventListener('click', toggleDeletions);
